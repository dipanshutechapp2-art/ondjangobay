...
trait WalletTransactionTrait {
    public function createWalletTransactionPayLink($output,$status = PaymentGatewayConst::STATUSSUCCESS) {
        ...
        $this->transactionChargePayLinkWallet($inserted_id,$output);
        $this->adminNotificationPayLinkWallet($trx_id,$output,$status,$receiver_user);
    ...
    public function insertRecordPayLinkWallet($output,$trx_id,$status){
        ...
        try{
            ...
            $this->transactionChargePayLinkWalletSender($sender_id,$output);
            //sender transactions end
            $this->updateWalletBalancePayLinkWallet($output);
            $this->updateWalletBalancePayLinkWalletSender($output);
    ...
    public function updateWalletBalancePayLinkWallet($output) {
    ...
    public function updateWalletBalancePayLinkWalletSender($output) {
        $update_amount = $output['sender_wallet']->balance - $output['charge_calculation']['sender_payable'];
        $output['sender_wallet']->update([
            'balance'   => $update_amount,
        ]);
    }
    public function transactionChargePayLinkWallet($id,$output) {
        DB::beginTransaction();
        try{
    ...
    public function transactionChargePayLinkWalletSender($id,$output) {
        DB::beginTransaction();
        try{
            DB::table('transaction_charges')->insert([
                'transaction_id'    => $id,
    ...
    public function adminNotificationPayLinkWallet($trx_id,$output,$status,$user){
