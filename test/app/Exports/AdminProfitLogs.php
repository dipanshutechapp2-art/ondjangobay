    ...
    public function array(): array
    {
        ...
        ->latest()->get()->map(function($item,$key){
            ...
            if($item->transactions->type == payment_gateway_const()::TYPEADDMONEY){
                $exchange_rate = $item->transactions->currency->rate ?? get_default_currency_rate();
            }else{
                $exchange_rate = $item->transactions->creator_wallet->currency->rate ?? get_default_currency_rate();
            }
            ...
            return [
                ...
                'profit_amount'  => get_amount($total_charge,get_default_currency_code(),get_wallet_precision()),
