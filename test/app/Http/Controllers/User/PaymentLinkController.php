...
{
    use StripeLinkPayment,WalletTransactionTrait,Authorize;
    /**
    ...
    */
    ...
    public function authorizeCardInfo(Request $request,$identifier){
        ...
        return view('payment-gateway.authorize',compact(
            'page_title',
            'temp_data'
        ));
    }
    ...
    public function authorizePaymentSubmit(Request $request,$identifier){
        ...
        $amount = floatval($temp_data->data->charge_calculation->requested_amount);
        ...
        $order->setDescription("Payment Link");

        // Set the customer's Bill To address
        $customerAddress = new AnetAPI\CustomerAddressType();
        $customerAddress->setFirstName($temp_data->data->validated->full_name ?? '');
        ...
        $make_customer_id       = rand(12,21) . $gateway_credentials->code;
        // Set the customer's identifying information
        $customerData = new AnetAPI\CustomerDataType();
        $customerData->setType("individual");
        $customerData->setId($make_customer_id);
        $customerData->setEmail($temp_data->data->validated->email);
