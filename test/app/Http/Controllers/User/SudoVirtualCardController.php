...
class SudoVirtualCardController extends Controller
{
    ...
    public function createPage(){
        ...
        $user               = userGuard()['user'];
        $cardCharge         = TransactionSetting::where('slug','virtual_card')->where('status',1)->first();
        $extra_fields_status    =  ($user->sudo_customer == null) ? true : false;

        return view('user.sections.virtual-card-sudo.create',compact('page_title','user','cardCharge','supported_currency','from_wallets','extra_fields_status'));
    }
    public function cardBuy(Request $request)
    {
        ...
        $validated = Validator::make($request->all(),[
            ...
            'date_of_birth'     => $extra_fields_status == true ? 'required|date' : 'nullable',
            'identity_type'     => $extra_fields_status == true ? 'required|string' : 'nullable',
            'identity_number'   => $extra_fields_status == true ? 'required|string|regex:/^[A-Z0-9]+$/i' : 'nullable',
            'phone_number'      => $extra_fields_status == true ? 'required|string|regex:/^\+?[0-9]{10,15}$/' : 'nullable',
        ...
        if(get_amount($amount, null,get_wallet_precision()) < get_amount($minLimit, null,get_wallet_precision()) || get_amount($amount, null,get_wallet_precision()) > get_amount($maxLimit, null,get_wallet_precision()))  return back()->with(['error' => [__("Please follow the transaction limit")]]);
        //daily and monthly
    ...
    public function cardFundPage($id){
        ...
        $supported_currency = support_currencies([$myCard->currency ?? "USD"], [card_currency_name($myCard->currency)]);
