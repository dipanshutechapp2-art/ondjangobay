...
class AgentCareController extends Controller
{
    ...
    public function create(){
        $page_title             = __("Create New Agent");

        return view('admin.sections.agent-care.create',compact(
            'page_title'
        ));
    }
    /**
    ...
    public function store(Request $request){
        ...
        $validated['mobile_code'] = ($val = trim($validated['mobile_code'] ?? '')) !== ''? remove_speacial_char($val) : null;
        $validated['mobile']      = ($val = trim($validated['mobile'] ?? '')) !== ''? remove_speacial_char($val): null;
        $validated['full_mobile'] = !empty($validated['mobile_code']) && !empty($validated['mobile']) ? $validated['mobile_code'] . $validated['mobile'] : null;

        $validated['username']      = $user_name;
        $validated['sms_verified']   = 1;
        $validated['password']      = Hash::make($validated['password']);

        $models = [User::class, Agent::class, Merchant::class];
        ...
        try{
            ...
            if($basic_settings->agent_email_notification){
                try{
                    Notification::route('mail',$validated['email'])->notify(new NewUserNotification($data = $validated,$request->password,"AGENT"));
                }catch(Exception $e){
                }
            }
        ...
        return redirect()->route('admin.agents.index')->with(['success' => [__('Agent created successfully.')]]);
    }
}
