...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $this_year = date('Y-m-d', strtotime('-1 year', strtotime(date('Y-m-d'))));
         ...
         $this_month_profits = agentOnBaseCurrency(TransactionCharge::with('transactions')
         ->whereHas('transactions', function ($query) {
             $query->where('status',1);
         })->whereBetween('created_at', [$this_month_start, $this_month_end])->get());


         $last_month_profits =agentOnBaseCurrency(TransactionCharge::with('transactions')
         ->whereHas('transactions', function ($query) {
             $query->where('status',1);
         }) ->whereBetween('created_at', [$last_month_start, $last_month_end])->get());
