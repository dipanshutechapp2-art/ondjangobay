...
class BillPayController extends Controller
{
    ...
    public function billPayConfirmed(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('user_id',$userWallet->user->id,PaymentGatewayConst::BILLPAY,$userWallet->currency,$amount,$billPayCharge,PaymentGatewayConst::SEND,null,$charges['exchange_rate']);
        }catch(Exception $e){
    ...
    public function automaticBillPay($request_data){
       ...
       if($bill_amount < $minLimit || $bill_amount > $maxLimit) {
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('user_id',$userWallet->user->id,PaymentGatewayConst::BILLPAY,$userWallet->currency,$bill_amount,$billPayCharge,PaymentGatewayConst::SEND,null,$charges['exchange_rate']);
        }catch(Exception $e){
            $errorData = json_decode($e->getMessage(), true);
            $error = ['error'=>[__($errorData['message'] ?? __("Something went wrong! Please try again."))]];
            return Helpers::error($error);
            ...
            try{
                if( $this->basic_settings->email_notification == true){
                    $notifyData = [
                        ...
                        'current_balance'   => getAmount($userWallet->balance,get_wallet_precision()),
