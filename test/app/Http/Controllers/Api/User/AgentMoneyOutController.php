...
class AgentMoneyOutController extends Controller
{
            ...
            $statusInfo = [
                ...
                if($item->attribute == payment_gateway_const()::SEND){
                    ...
                    return[
                        ...
                        'payable' => getAmount(@$item->request_amount,get_wallet_precision()).' '.get_default_currency_code(),
                        'recipient_received' => getAmount(@$item->details->recipient_amount,get_wallet_precision()).' '.get_default_currency_code(),
                        'current_balance' => getAmount(@$item->available_balance,get_wallet_precision()).' '.get_default_currency_code(),
                        'status' => @$item->stringStatus->value ,
                        'date_time' => @$item->created_at ,
    ...
    public function confirmed(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('user_id',$sender_wallet->user->id,PaymentGatewayConst::AGENTMONEYOUT,$sender_wallet->currency,$validated['amount'],$trx_charges,PaymentGatewayConst::SEND);
        }catch(Exception $e){
