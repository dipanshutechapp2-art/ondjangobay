...
class ReceiveMoneyController extends Controller
{
    ...
    public function refundBalanceForMakePayment(Request $request) {
        ...
        $user  = User::where('username',$details->sender->username)->first();
        if(!$user){
            $error = ['error'=>[__("Oops! User not exists")]];
            return Helpers::error($error);
        }


        $user_wallet = UserWallet::where('user_id',$user->id)->whereHas("currency",function($q) use ($transaction) {
            $q->where("code",$transaction->creator_wallet->currency->code ?? get_default_currency_code())->active();
        })->active()->first();
        if(!$user){
        ...
        $refundableBalance          = $transaction->details->sender_amount;
        $merchantReducibleBalance   = $user_transaction->details->recipient_amount;
        ...
        try{
            ...
            //admin notification
            $this->adminNotifications($transaction,$user_transaction);
        }catch(Exception $e){
            $error = ['error'=>[__("Something went wrong! Please try again.")]];
            return Helpers::error($error);

        }
        $message =  ['success'=>[__('Refund processed successfully')]];
    ...
    public function refundBalanceForPaymentGateway(Request $request) {
        ...
        try{
            ...
            //send user notification
            $this->sendUserNotificationCheckoutPay($user,$user_transaction,$basic_settings);

            //send merchant notification
            $this->sendMerchantNotificationCheckoutPay($merchant,$transaction,$basic_settings);

            //admin notification
            $this->adminNotificationsCheckoutPay($transaction);
    ...
    //***************************************Helper Functions For Make Payment*********************************************************/
    public function sendUserNotification($user,$user_transaction,$basic_settings){
        try{
            ...
            $notification_content = [
                ...
                'message'       => "From MERCHANT("." @".$details->receiver->username .") "." ".get_amount($user_transaction->request_amount,get_default_currency_code(),get_wallet_precision())." "."Successful."."TRX ID ".$user_transaction->trx_id.".",
    ...
    public function sendMerchantNotification($merchant,$transaction,$basic_settings){
        ...
        try{
            ...
            $notification_content = [
                'title'         => "Refund Balance",
                'message' => "Successfully sent " .get_amount($transaction->request_amount,get_default_currency_code(),get_wallet_precision()) .
             " to USER (@" . $details->sender->username . "). TRX ID: " . $transaction->trx_id . ".",
    ...
    public function adminNotifications($transaction,$user_transaction){
        $details        = $transaction->details;
        $trx_id         = $transaction->trx_id;
        $user_details   = $user_transaction->details;

        $notification_content = [
            //email notification
            'subject' =>__("Refund Balance"),
            'greeting' =>__("Refund Balance Information"),
            'email_content' =>__("web_trx_id")." : ".$trx_id."<br>".__("sender").": @".$user_details->receiver->username."<br>".__("Receiver").": @".$details->sender->username."<br>".__("Refund Balance")." : ".get_amount($transaction->request_amount, get_default_currency_code(), get_wallet_precision()),

            //push notification
            'push_title' => __("Refund Balance")." ".__('Successful'),
            'push_content' => __('web_trx_id')." ".$trx_id.",".__("sender").": @".$user_details->receiver->username.",".__("Receiver").": @".$details->sender->username.",".__("Refund Balance")." : ".get_amount($transaction->request_amount, get_default_currency_code(), get_wallet_precision()),

            //admin db notification
            'notification_type' =>  NotificationConst::REFUND,
            'trx_id' =>  $trx_id,
            'admin_db_title' => "Refund Balance"." ".'Successful'." ".get_amount($transaction->request_amount, get_default_currency_code(), get_wallet_precision())." (".$trx_id.")",
            'admin_db_message' =>"Sender".": @".$user_details->receiver->username.","."Receiver".": @".$details->sender->username.","."Refund Balance"." : ".get_amount($transaction->request_amount, get_default_currency_code(), get_wallet_precision())
        ];
    ...
    //***************************************Helper Functions Merchant Payment*********************************************************/
    public function sendUserNotificationCheckoutPay($user,$user_transaction,$basic_settings){
        try{
            $details = $user_transaction->details;
            // email notification
            if( $basic_settings->email_notification == true){
    ...
    public function sendMerchantNotificationCheckoutPay($merchant,$transaction,$basic_settings){
    ...
    public function adminNotificationsCheckoutPay($transaction){
        $details = $transaction->details;
        $trx_id  = $transaction->trx_id;
        $notification_content = [
            //email notification
