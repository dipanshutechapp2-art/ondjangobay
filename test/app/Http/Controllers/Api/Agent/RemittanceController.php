...
class RemittanceController extends Controller
{
    ...
    public function remittanceInfo(){
        ...
        $fromCountry = Currency::default()->get()->map(function($data){
            return[
                ...
                'symbol' => $data->symbol,
                'flag' => $data->flag,
                'rate' => getAmount( $data->rate,get_wallet_precision()),
                'status' => $data->status,
    ...
    public function confirmed(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('agent_id',$userWallet->agent->id,PaymentGatewayConst::SENDREMITTANCE,$userWallet->currency,$validated['send_amount'],$exchangeCharge,PaymentGatewayConst::SEND);
        }catch(Exception $e){
        ...
        if($charges->payable > $userWallet->balance) {
         ...
         $notifyData = [
            ...
            'request_amount'  => getAmount($charges->sender_amount,get_wallet_precision()).' '.$charges->sender_cur_code,
            'exchange_rate'  => "1 " .get_default_currency_code().' = '.get_amount($charges->exchange_rate,$charges->receiver_cur_code,get_wallet_precision()),
            'charges'   => getAmount( $charges->total_charge,get_wallet_precision()).' ' .$charges->sender_cur_code,
            'payable'   =>  getAmount($charges->payable,get_wallet_precision()).' ' .$charges->sender_cur_code,
            ...
            'receiver_get'   =>  getAmount($charges->will_get,get_wallet_precision()).' ' .$charges->receiver_cur_code,
            'status'  => __("Pending"),
          ];
        try{
