...
class SendMoneyController extends Controller
{
    ...
    public function sendMoneyInfo(){
        ...
        $sendMoneyCharge = TransactionSetting::where('slug','transfer')->where('status',1)->get()->map(function($data){
            return[
                ...
                'agent_fixed_commissions'   => getAmount($data->agent_fixed_commissions,get_wallet_precision()),
                'agent_percent_commissions' => getAmount($data->agent_percent_commissions,get_wallet_precision()),
            ...
            $statusInfo = [
                ...
                if($item->attribute == payment_gateway_const()::SEND){
                    return[
                        ...
                        'request_amount' => getAmount(@$item->request_amount,get_wallet_precision()).' '.$item->details->charges->sender_currency ,
                        'total_charge' => getAmount(@$item->charge->total_charge,get_wallet_precision()).' '.$item->details->charges->sender_currency,
                        'payable' => getAmount(@$item->payable,get_wallet_precision()).' '.$item->details->charges->sender_currency,
                        'recipient_received' => getAmount(@$item->details->charges->receiver_amount,get_wallet_precision()).' '.$item->details->charges->receiver_currency,
    ...
    public function confirmedSendMoney(Request $request){
        ...
        try{
            (new TransactionLimit())->trxLimit('agent_id',$sender_wallet->agent->id,PaymentGatewayConst::TYPETRANSFERMONEY,$sender_wallet->currency,$validated['amount'],$trx_charges,PaymentGatewayConst::SEND);
        }catch(Exception $e){
                 ...
                 try{
                    if( $basic_setting->agent_email_notification == true){
                        $notifyDataSender = [
                            ...
                            'request_amount'  => getAmount($charges['sender_amount'],get_wallet_precision()).' '.$charges['sender_currency'],
                            'payable'   =>  getAmount($charges['payable'],get_wallet_precision()).' ' .$charges['sender_currency'],
                            'charges'   => getAmount( $charges['total_charge'],get_wallet_precision()).' ' .$charges['sender_currency'],
                            'received_amount'  => getAmount($charges['receiver_amount'],get_wallet_precision()).' ' .$charges['receiver_currency'],
                            'status'  => __("success"),
                        ];
