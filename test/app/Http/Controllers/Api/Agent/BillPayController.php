...
class BillPayController extends Controller
{
    ...
    public function billPayInfo(){
        ...
        foreach ($billTypeArray as &$item) {
            ...
            $item['receiver_currency_rate'] = getAmount(get_default_currency_rate(),get_wallet_precision());
        ...
        $transactions = Transaction::agentAuth()->billPay()->latest()->take(5)->get()->map(function($item){
            ...
            return[
                ...
                'total_charge' => getAmount($item->charge->total_charge,get_wallet_precision()).' '.billPayCurrency($item)['wallet_currency'],
                'current_balance' => getAmount($item->available_balance,get_wallet_precision()).' '.billPayCurrency($item)['wallet_currency'],
    ...
    public function billPayConfirmed(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('agent_id',$sender_wallet->agent->id,PaymentGatewayConst::BILLPAY,$sender_wallet->currency,$validated['amount'],$trx_charges,PaymentGatewayConst::SEND,null,$charges['exchange_rate']);
        }catch(Exception $e){
    ...
    public function automaticBillPay($request_data){
       ...
       if($bill_amount < $minLimit || $bill_amount > $maxLimit) {
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('agent_id',$sender_wallet->agent->id,PaymentGatewayConst::BILLPAY,$sender_wallet->currency,$bill_amount,$billPayCharge,PaymentGatewayConst::SEND,null,$charges['exchange_rate']);
        }catch(Exception $e){
            $errorData = json_decode($e->getMessage(), true);
            $error = ['error'=>[__($errorData['message'] ?? __("Something went wrong! Please try again."))]];
            return Helpers::error($error);
            ...
            try{
                if( $this->basic_settings->email_notification == true){
                    $notifyData = [
                        ...
                        'exchange_rate'       => get_amount(1,$charges['sender_currency'])." = ".get_amount($charges['exchange_rate'],$charges['wallet_currency'],get_wallet_precision()),
                        ...
                        'current_balance'   => getAmount($sender_wallet->balance,get_wallet_precision()),
