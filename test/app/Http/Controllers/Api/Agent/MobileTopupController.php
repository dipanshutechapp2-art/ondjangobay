...
class MobileTopupController extends Controller
{
    ...
    public function topUpInfo(){
        ...
        $transactions = Transaction::agentAuth()->mobileTopup()->latest()->take(5)->get()->map(function($item){
            ...
            return[
                ...
                'total_charge'          => getAmount($item->charge->total_charge,get_wallet_precision()).' '.topUpCurrency($item)['wallet_currency'],
                'current_balance'       => getAmount($item->available_balance,get_wallet_precision()).' '.topUpCurrency($item)['wallet_currency'],
    ...
    public function payAutomatic(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('agent_id',$sender_wallet->agent->id,PaymentGatewayConst::MOBILETOPUP,$sender_wallet->currency,$validated['amount'],$topupCharge,PaymentGatewayConst::SEND,null,$charges['exchange_rate']);
        }catch(Exception $e){
            $errorData = json_decode($e->getMessage(), true);
            $error = ['error'=>[__($errorData['message'] ?? __("Something went wrong! Please try again."))]];
            return Helpers::error($error);
        ...
        try{
            ...
            if($this->basic_settings->agent_email_notification == true){
                ...
                $notifyData = [
                    ...
                    'exchange_rate'     => get_amount(1,$charges['destination_currency'])." = ".get_amount($charges['exchange_rate'],$charges['sender_currency'],get_wallet_precision()),
