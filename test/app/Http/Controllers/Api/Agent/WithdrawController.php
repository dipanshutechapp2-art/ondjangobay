...
class WithdrawController extends Controller
{
    ...
    public function withdrawInfo(){
        ...
        $data =[
            ...
            'base_curr_rate'        => getAmount(1,get_wallet_precision()),
    ...
    public function withdrawInsert(Request $request){
        ...
        //daily and monthly
        try{
            (new TransactionLimit())->trxLimit('agent_id',$agentWallet->agent->id,PaymentGatewayConst::TYPEMONEYOUT,$agentWallet->currency,$amount,$gate,PaymentGatewayConst::SEND);
        }catch(Exception $e){
    ...
    public function confirmWithdrawAutomatic(Request $request){
        ...
        //flutterwave automatic
         if($track->data->gateway_name == "Flutterwave"){
            ...
            return $this->flutterwavePay($gateway,$request,$track,$callback_url,$reference);
         }else{
            $error = ['error'=>[__("Something went wrong! Please try again.")]];
            return Helpers::error($error);
         }
    ...
    //fluttrwave
    public function flutterwavePay($gateway,$request, $track,$callback_url,$reference){
        $withdrawdata =  $track->data;
        $basic_setting = BasicSettings::first();
