...
@section('content')
	...
	<script>
		$(document).ready(function () {
			$('#categoryTable').DataTable({
				processing: true,
				serverSide: true,
				ajax: '{{ url('vendor/product-brands') }}',
				columns: [
					{ data: 'title' },
					{ data: 'status' },
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('switchChange.bootstrapSwitch', '.category-status-switch', function(event, state) {
				...
				$.ajax({
					url: '{{ route('product.brands.updateStatus') }}',
					type: 'POST',
					data: {
						_token: '{{ csrf_token() }}',
						id: categoryId,
    ...
    <script>
        $(document).on('click', '.delete-category', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "You are about to delete this Brand. Every informtation under this Brand will be deleted.",
                icon: 'warning',
...
$(document).on('click', '.edit-btn', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: '{{ url('vendor/product-brands') }}/' + categoryId,
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    _method: 'PUT',
                ...
                success: function (response) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Updated',
                        text: 'brand name updated successfully',
                        timer: 2000,
                        showConfirmButton: false
