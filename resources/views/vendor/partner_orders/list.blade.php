    ...
    <div class="content-wrapper admin-dashboard-content">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>Campgain Orders</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{ url('/vendor/dashboard') }}">Home</a></li>
                            <li class="breadcrumb-item active">Campgain Orders</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                    ...
                    <div class="col-12">
                        <div class="card">
                           <div class="card-header">
                                <h3 class="card-title">Campgain Orders </h3>
										...
										@if(isset($orders) && count($orders))
											@foreach($orders as $order)
												<tr>
													...
													<td> <a href="{{url('/vendor/partner-order/product-order-details')}}/{{$order->id}}" class="btn btn-success btn float-right">View</a><br/><br/>
													<a href="{{ route('vendor.partner.order.invoice.download', $order->id) }}" class="btn btn-primary">Invoice</a></td>
												</tr>
											@endforeach
										@endif
									</tbody>
        ...
        $(document).on('click', '.delete-enquiry', function() {
            ...
            <script>
                $(document).off('change', '.order-status-dropdown').on('change', '.order-status-dropdown', function() {
                    ...
                    $.ajax({
                        url: '{{ url("/vendor/partner-product-orders/update-status") }}',
                        type: 'POST',
                        data: {
                            order_id: orderId,
                            order_status: status,
			...
			<script>
                $(document).off('change', '.payment-status-dropdown').on('change', '.payment-status-dropdown', function() {
                    ...
                    $.ajax({
                        url: '{{ url("/vendor/partner-product-orders/payment-status") }}',
                        type: 'POST',
                        data: {
                            order_id: orderId,
