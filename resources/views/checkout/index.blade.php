...
<style>
	/* Fix for Bootstrap + Intl-Tel-Input alignment */
	.iti {
		width: 100%;
		display: block;
	}

	/* Ensures the dropdown list appears above other elements */
	.iti__country-list {
		z-index: 100;
	}
</style>
<main class="main checkout">
	...
	<div class="page-content">
		<div class="container">
			...
			<form class="login-content" id="ajax-login-form" method="POST" style="display: none; overflow: hidden;">
				@csrf
				<p>
			...
			<div class="coupon-content mb-4" style="display: none;"> <!-- Changed to display: block -->
				@if(session('coupon'))
			...
			<div class="mb-3">
				<div class="form-check">
					<!--<input type="checkbox" class="form-check-input" id="guest_checkout" name="guest_checkout" required>-->
					<label class="form-check-label font-weight-bold" for="guest_checkout">
						Checkout as Guest
					</label>
				</div>
				<small class="text-muted">Continue without creating an account</small>
			</div>
			@endif
								...
								<div class="col-md-6">
									...
									<div class="form-group">
										...
										<input type="tel"
											...
											style="padding-left: 90px;"> <input type="hidden" name="billing_phone" id="hidden_phone" value="{{ auth()->user()->phone ?? old('billing_phone') }}">
								...
								<table class="order-table">
									...
									<tfoot>
										...
										<tr>
											...
											@if(!empty($cartItems)) 
												<td><b>{{ formatCurrency($shippingPrice) }}</b></td>
											@else
										...
										<tr class="order-total">
											<th><b>Total</b></th>
											@if(session('coupon')  && $cartItems)
												<td><b>{{formatCurrency(($subtotal+$shippingPrice)-session('coupon.discount'))}}</b></td>
											@else
												@if(!empty($cartItems)) 	
													<td><b>{{formatCurrency($subtotal+$shippingPrice)}}</b></td>
												@else
													<td><b>{{formatCurrency(0)}}</b></td>
												@endif
											@endif
...
<script>
	...
	var hiddenInput = document.querySelector("#hidden_phone");

	var iti = window.intlTelInput(input, {
		initialCountry: "auto",
		separateDialCode: true, // This shows the Flag + Code (+91) separately
		...
		utilsScript: "https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.19/js/utils.js" // Format script
	});

	// Logic: Update hidden input with full number before submit
	// Jab user type karega, hidden input me Full Number (+91...) save hoga
	input.addEventListener('input', function() {
		hiddenInput.value = iti.getNumber();
	});

	// Ensure value is set correctly on load if user is editing
	if (input.value) {
		hiddenInput.value = iti.getNumber();
	}
</script>
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		$('#ajax-login-form').submit(function(e) {
			...
			const form = $(this);
			const url = "{{ route('user.login.checkout') }}";
