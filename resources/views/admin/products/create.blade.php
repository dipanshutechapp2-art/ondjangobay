										  ...
										  <div class="card-body">
											<form class="geniusform"  onsubmit="return validateBeforeSubmit()"  action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
											 @csrf   
												...
												<hr/>
												<div class="form-group">
												<sup><b>(If you want to add varient option then select vendor first.)</b></sup>
													<div id="attribute-blocks"></div>
													<button type="button" id="defaultDisabled" disabled class="btn btn-secondary mb-3" onclick="addAttributeGroup()">+ Add Attribute</button>
												</div>
												...
												<div class="form-group">
													<label for="inp-email">Store <span style="color:red;">*</span> <sup><b>(If you want to select store then will be select vendor first.)</b></sup></label>
													<select name="store_ids[]" disabled class="form-control select2Stores" multiple required>
											   ...
											   <div class="form-group">
												...
												<div class="form-group">
													...
													<textarea class="form-control" id="summernote" name="short_description" placeholder="Short description">{{ old('short_description  ') }}</textarea>
												...
												<div class="form-group">
													...
													<textarea class="form-control" id="summernote2" name="description" placeholder="Description">{{ old('description  ') }}</textarea>
													@error('description')
														<span class="invalid-feedback" role="alert">
															<strong>{{ $message }}</strong>
														</span>
	...
	<script>
		...
		if (vendorID) {	
            ...
            .then(data => {
                ...
                if ($(storeSelect).hasClass('select2')) {
                    $(storeSelect).trigger('change.select2');
                }
            })
            .catch(error => {
                console.error('Error fetching stores:', error);
                storeSelect.innerHTML = '<option disabled>No stores for this vendor</option>';
            });
