    ...
    @if(isset($orders) && count($orders))
        @foreach($orders as $order)
            <tr>
				...
				<td> <a href="{{url('/admin/order/order-details')}}/{{$order->id}}" class="btn btn-success btn float-right">View</a><br/><br/>
				    <a href="{{ route('order.invoice.download', $order->id) }}" class="btn btn-primary">Invoice</a>
				</td>
            </tr>
        @endforeach
    @endif
    ...
    <script>
		...
		<script>
                $(document).off('change', '.payment-status-dropdown').on('change', '.payment-status-dropdown', function() {
                    ...
                    $.ajax({
                        url: '{{ url("/admin/orders/payment-status") }}',
                        type: 'POST',
                        data: {
                            order_id: orderId,
