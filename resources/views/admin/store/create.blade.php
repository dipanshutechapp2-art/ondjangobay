                                    ...
                                    <div class="col-md-12">
                                        <div class="card mb-4">
										  <div class="card-header">
											...
											@endif
										  <div class="card-body">
											<form class="geniusform" action="{{ route('admin.vendorstore.create') }}" method="POST" enctype="multipart/form-data">
											 @csrf 
                                                @method('POST')		
													...
													<label for="inp-name">Vendor <span style="color:red;">*</span></label>
													  <select class="form-control @error('user_id') is-invalid @enderror" name="user_id" required>
															...
															@foreach($vendors as $vendor)
																<option value="{{ $vendor->id }}" {{ old('user_id') == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}</option>
															@endforeach		
													  </select>
													@error('user_id')	
