...
<head>
    ...
    <link rel="preload" href="{{ asset('frontend/assets/fonts/excs-1.woff?png09e')}}" as="font" type="font/woff" crossorigin="anonymous">
    ...
    <!-- Plugin CSS -->
    <link rel="stylesheet" href="{{ asset('frontend/assets/vendor/swiper/swiper-bundle.min-1.css')}}">
    <!--<link rel="stylesheet" type="text/css" href="{{ asset('frontend/assets/vendor/magnific-popup/magnific-popup.min-1.css')}}">-->

    <!-- Default CSS -->
    <link rel="stylesheet" type="text/css" href="{{ asset('frontend/assets/css/style_inner.css')}}?v={{ time() }}">
...
@endphp
    <div class="page-wrapper">
            ...
            <div class="header-middle">
                <div class="container">
                    <div class="header-left mr-md-4">
                        <a href="{{url('/')}}"" class="mobile-menu-toggle  w-icon-hamburger" aria-label="menu-toggle">
                        </a>
                        <a href="{{url('/')}}" class="logo ml-lg-0">
                            <!--<img src="{{ asset('uploads/setting') }}/{{ $setting->header_logo }}" alt="logo" width="144" height="45">-->
                            <img src="{{ asset('uploads/setting') }}/{{ $setting->footer_logo }}" alt="logo" width="230">
                        </a>
                         <form method="get" action="{{url('/shop')}}" class="header-search hs-expanded hs-round bg-white br-xs d-md-flex input-wrapper mr-2 ml-2 inner-page-search">
							@php 	
                            ...
                            <input type="text" class="form-control" name="search" id="ajax-search" value="{{ request('search') }}" autocomplete="off" placeholder="Search in...">
                    ...
                    <div class="header-right ml-4 d-none">
                ...
                <div class="container">
                    <div class="inner-wrap">
                        <div class="header-left">
                            <div class="dropdown category-dropdown has-border d-none" data-visible="true">
													...
													<ul class="megamenu  megamenu-ul">
														@foreach($category->children as $child)
															<li class="megamenu-li">
																...
																@else
																	<a href="{{ url('/shop') }}/?category={{$child->id}}">
																	<img src="{{ asset('/frontend/assets/no-image.webp') }}"
												 alt="{{ $child->name }}" width="80" height="80"></a>
																@endif
																	<h4 class="menu-title megamenu-title-list">
                        ...
                        <div class="header-right">
                            @if(is_track_order()>0)
							   <a href="{{url('/')}}" class="d-xl-show"><i class="w-icon-map-marker mr-1"></i>Track Order</a>
						    @endif
		...
		<!-- Start of Footer -->
        <!-- Start of Footer -->
        <footer class="footer" style="padding-bottom: 0rem;">
...
"></div>
	...
	<script src="{{ asset('frontend/assets/vendor/sticky/sticky-1.js')}}"></script>
    ...
    <script src="{{ asset('frontend/assets/vendor/zoom/jquery.zoom-1.js')}}"></script>
	 <script src="{{ asset('frontend/assets/vendor/photoswipe/photoswipe-1.js')}}"></script>
    <script src="{{ asset('frontend/assets/vendor/photoswipe/photoswipe-ui-default-1.js')}}"></script>
    <!--<script src="{{ asset('frontend/assets/vendor/jquery.countdown/jquery.countdown.min-1.js')}}"></script>-->
	 <!-- Swiper JS -->
    <script src="{{ asset('frontend/assets/vendor/swiper/swiper-bundle.min-1.js')}}"></script>

    <script src="{{ asset('frontend/assets/js/main.min-1.js') }}?v={{ time() }}"></script>
...
<script>
document.addEventListener('DOMContentLoaded', () => {
  const toggles = document.querySelectorAll('.widget-collapsible .toggle-category');
  ...
  toggles.forEach(toggle => {
    toggle.addEventListener('click', function (e) {
      ...
      const submenu = parentLi.querySelector('.subcategoryFilter');
      const icon = this.querySelector('.toggle-icon');
      const isOpen = submenu.classList.contains('show');

      // Close all submenus and remove rotations
      document.querySelectorAll('.widget-collapsible .category-item.has-children').forEach(item => {
        const sub = item.querySelector('.subcategoryFilter');
        const icn = item.querySelector('.toggle-icon');
        item.classList.remove('expanded');
        if (sub) sub.classList.remove('show');
        if (icn) icn.classList.remove('rotated');
      });

      // If clicked submenu was closed, open it
      if (!isOpen) {
