...
<script type="module">
	...
	<!-- Start of Main -->
        ...
        {{ __('Log in easily—just enter your email and password to continue where you left off.') }}
								...
								<form class="form contact-us-form" id="login-form" action="{{ route('user.login') }}" method="post">
									...
									<div class="form-group">
										...
										<x-text-input id="identifier" class="block mt-1 w-full form-control"
											...
											<span id="identifier-error" style="color:red;" class="text-red-500 text-sm"></span>
									</div>
									...
									<div class="form-group">
										...
										<x-text-input id="password-login" class="block mt-1 w-full form-control"
											...
											<span id="password-error" style="color:red;" class="text-red-500 text-sm"></span>
									</div>
							...
							<!--<div class="col-md-6">
							    <div class="register-form-box">
							        	<h4 class="justify-content-center mb-1">REGISTER HERE</h4>
...
<script type="module">
  ...
  const firebaseConfigApple = {
    ...
    authDomain: "exco-e7b3f.firebaseapp.com",
    projectId: "exco-e7b3f",
    appId: "1:324522148063:web:2a639b25735316fc15e96a",
  };

  const app = initializeApp(firebaseConfigApple);
  const auth = getAuth(app);
  ...
  document.getElementById('appleLoginBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const response = await fetch('{{ url("/auth/apple/firebase") }}', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': token
...
var loginWidgetId, registerWidgetId;
...
function onRecaptchaLoad() {
    ...
    console.log('reCAPTCHA widgets rendered:', {loginWidgetId, registerWidgetId});
}
...
$('#login-form input').on('blur keyup', function() { 
    ...
    $.ajax({
        ...
        error: function (xhr) {
            ...
            $("#identifier-error").text(errors?.identifier ? errors.identifier[0] : "");
