...
class CurrencyController extends Controller
{
	...
	public function index(Request $request){
		...
		if ($request->ajax()) {
			...
			$currency = Currency::orderBy('display_order','ASC')->get();
			...
			return DataTables::of($currency)
				...
				->addColumn('action', function($currency) {
						...
						$editUrl   = url('/admin/currency/edit', $currency->id);
						$deleteUrl = url('/admin/currency/delete', $currency->id);
						$token     = csrf_token();
	...
	public function add_currency_action(Request $request){ 
		...
		$currency->display_order = $request->display_order;
		$currency->save();
		...
		return redirect('/admin/currency')->with('success', 'You have successfully added!');
    }
	
	public function edit_currency($currency_id){
		
		$currencyInfo = Currency::where('id',$currency_id)->first();
		return view('admin.currency.edit',compact('currencyInfo'));
	...
	public function edit_currency_action(Request $request){
        ...
        return redirect('/admin/currency')->with('success', 'You have successfully updated!');
    }
	
	public function delete_currency($currency_id){
	
		Currency::where('id',$currency_id)->delete();
		return redirect('/admin/currency')->with('success', 'You have successfully deleted!');
    }
