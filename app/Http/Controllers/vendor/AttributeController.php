...
class AttributeController extends Controller
{
    public function index(Request $request) {
		...
		if ($request->ajax()) {
			...
			$attributes = Attribute::with('values','vendor')->where('vendor_id',auth()->id())->get();
			...
			return DataTables::of($attributes)
				...
				->addColumn('action', function($attributes) {
					...
					$editUrl   = route('attributes.edit', $attributes->id);
					$deleteUrl = route('attributes.destroy', $attributes->id);
					$token = csrf_token();
		...
		return view('vendor.attributes.list');
    }
    ...
    public function create() {
		...
		return view('vendor.attributes.create',compact('vendors'));
    }
    ...
    public function edit($id) {
        ...
        return view('vendor.attributes.edit', compact('attributes','vendors'));
    }
        ...
        try {
			...
			$data = $request->validate([
				...
				'name'       => ['required','string',
									...
									->where(fn ($query) => $query->where('vendor_id', auth()->id()))
									->ignore($id),
								],			
                'values'     => 'required|array|min:1',
				'values.*'   => 'required|string',
