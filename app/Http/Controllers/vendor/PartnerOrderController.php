...
class PartnerOrderController extends Controller
{
    public function index() {
        ...
        $orders = PartnerOrder::with('user')->where('vendor_id',auth()->id())->orderBy('created_at', 'desc')->paginate(20);

        return view('vendor.partner_orders.list', compact('orders'));
    }
	
	public function orderDetails(Request $request,$id) { 
	
	    $orderInfo = PartnerOrder::with('product')->where('id',$id)->where('vendor_id',auth()->id())->first();
	
		return view('vendor.partner_orders.order_details',compact('orderInfo'));
    }
	...
	public function downloadInvoice($id)
    {
        ...
        $pdf = Pdf::loadView('vendor.invoices.partner_invoice', compact('orderInfo'));
