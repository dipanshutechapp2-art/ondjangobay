	...
	private function createAutoDSProduct(
		...
		if (!empty($item['images']) && is_array($item['images'])) {
			...
			foreach ($item['images'] as $imageUrl) {
				...
				$response = Http::get($imageUrl['url']);
				if ($response->successful()) {
            ...
            foreach ($grouped as $vendorId => $items) {
				...
				foreach ($items as $item) {
					...
					$payload = [
						...
						'suggested_buy_price'    => $buyPrice ?? 0, 
						
						//'buy_currency'         => 'USD',
						//'active_buy_item.item_id_on_site'      => $sellSiteOrderId,
						//'sell_site'              => $variant->auto_ds_buy_item_url,
						//'sell_variant_id'        => $variant->auto_ds_variant_id,
						//'sell_variant_sku'       => $variant->sku,
						//'sell_item_id'           => $product->auto_ds_product_id,
						//'breakeven'              => 20, //5-20
						//'profit'                 => (float) ($item->price - $variant->price),
						
						'sell_price'             => round($sellUnitPrice * $quantity, 2),
						'buyer_email'    	     => $order->email,
